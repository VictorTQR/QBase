...
> **密钥来源升级（已实施，见 §19）**：当前版本实际 Key 读取优先级为「进程环境变量 → `.knowledge/secrets.toml` 的 `[keys]` → `config.toml` 明文 `api_key`（仅兼容）」。已支持知识库目录内的本地密钥文件 `.knowledge/secrets.toml`，由应用读取，`api_key_env` 表示「密钥名称」而非强制系统环境变量。
...
✅ M5：LanceDB 向量搜索
✅ M6：AI 总结生成
✅ M7：统一导航 + 设置页 + 任务中心增强 + 配置 UI 化 + secrets.toml 本地密钥方案（本阶段，详见 §19）
...
API Key 本地密钥文件方案（`.knowledge/secrets.toml`）已与 qwen 讨论、形成设计并完成开发，作为 M7 配置体系的安全增强，详见 §19，不占用新的整数里程碑。
...
## 19. API Key 本地密钥文件方案（.knowledge/secrets.toml，已实施）
