...
```javascript
{
  ...
  total_keywords: number,      // 关键词总数
  recent_papers: number        // 最近论文数量
}
```
...
| `/api/papers/import` | POST | 通过论文 ID 导入单篇论文 |
| `/api/papers/paper/{entry_id}` | GET | 获取单篇已保存的论文 |
| `/api/papers/paper/{entry_id}` | DELETE | 删除已保存的论文 |
...
**获取论文详情**:
```
GET /api/papers/paper/2301.12345
```

**删除论文**:
```
DELETE /api/papers/paper/2301.12345
```
