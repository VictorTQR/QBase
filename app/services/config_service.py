...
def get_analysis_llm_config() -> dict:
    """读取 AI 深度分析配置（[llm.analysis]，m18；enabled=false 时返回未启用状态，不校验）。"""
    config = load_config()
    analysis_config = config.get("llm", {}).get("analysis", {})

    enabled = bool(analysis_config.get("enabled", False))

    base_url = str(analysis_config.get("base_url", "")).strip()
    api_key_env = str(analysis_config.get("api_key_env", "")).strip()
    api_key = str(analysis_config.get("api_key", "")).strip()
    ...
    result = {
        "enabled": enabled,
        "provider": str(analysis_config.get("provider", "openai_compatible")),
        "base_url": base_url,
        "api_key_env": api_key_env,
        "api_key": api_key,
        "model": str(analysis_config.get("model", "")).strip(),
        "temperature": float(analysis_config.get("temperature", 0.3)),
        "max_tokens": int(analysis_config.get("max_tokens", 6000) or 6000),
        "timeout": int(analysis_config.get("timeout", 600) or 600),
        "max_input_chars": int(
    ...
    if enabled:
        if not base_url:
            raise ValueError("AI 分析配置缺少 base_url")

        if not result["model"]:
            raise ValueError("AI 分析配置缺少 model")
        ...
        if not api_key:
            ...
            raise ValueError("未获取到 AI 分析 API Key，请配置 api_key_env 或 api_key")
...
def _mask_api_keys(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for section in ("embedding", ("llm", "summary"), ("llm", "tagging"), ("llm", "analysis")):
        node = masked
...
def validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if analysis.get("enabled"):
        if not str(analysis.get("base_url", "")).strip():
            raise ConfigError("AI 分析已启用，但 base_url 为空")

        if not str(analysis.get("model", "")).strip():
            raise ConfigError("AI 分析已启用，但 model 为空")

        analysis_max_tokens = int(analysis.get("max_tokens", 6000))
        if analysis_max_tokens <= 0:
            raise ConfigError("AI 分析 max_tokens 必须大于 0")

        analysis_timeout = int(analysis.get("timeout", 600))
        if analysis_timeout <= 0:
            raise ConfigError("AI 分析 timeout 必须大于 0")

        analysis_max_input = int(analysis.get("max_input_chars", 100000))
        if analysis_max_input <= 0:
            raise ConfigError("AI 分析 max_input_chars 必须大于 0")
...
def get_key_status(config: dict[str, Any] | None = None) -> dict[str, dict]:
    ...
    llm_analysis = config.get("llm", {}).get("analysis", {})
    if llm_analysis.get("api_key_env"):
        key_names.add(str(llm_analysis["api_key_env"]))
...
def has_plain_api_key(config: dict[str, Any] | None = None) -> bool:
    ...
    llm = config.get("llm", {})
    llm_key = (
        llm.get("summary", {}).get("api_key")
        or llm.get("tagging", {}).get("api_key")
        or llm.get("analysis", {}).get("api_key")
    )
...
def test_connection(
    ...
    if kind == "llm_analysis":
        return _test_llm_connection(
            config.get("llm", {}).get("analysis", {}), "AI 分析"
        )
